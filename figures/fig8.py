...
for i in range(len(clean_pumping)):
    plt.plot(x_dates_clean[i],150+50*clean_pumping[i],'k')

plt.plot(ion_xdates,np.array(ion_refl)/1000,'b-')
plt.plot(ion_xdates,np.array(ion_b)/1000,'b--')
...
ax.set_ylim(190000/1000,290000/1000)
plt.xlabel("Время ЧЧ:ММ, UTC")
plt.ylabel("$h_\mathrm{М}$, км")
plt.grid()
...
plt.legend([h_el,h_sp,h_dr],['M3','M4','M5'],loc=(0.13,0.15))


# plt.title("Airglow's center height 140824")
plt.savefig("fig8_1.png",dpi=300)
plt.savefig("fig8_1.pdf",dpi=300)
plt.savefig("fig8_1.eps",dpi=300)
plt.close()
# plt.show()
...
for i in range(len(clean_pumping)):
    plt.plot(x_dates_clean[i],220+10*clean_pumping[i],'k')

plt.plot(ion_xdates[0:166],np.array(ion_refl[0:166])/1000,'b-')
plt.plot(ion_xdates[0:166],np.array(ion_b[0:166])/1000,'b--')
# plt.plot(ion_xdates,np.array(ion_t)/1000,'g-')
plt.plot(ion_xdates[166::],np.array(ion_refl[166::])/1000,'b-')
plt.plot(ion_xdates[166::],np.array(ion_b[166::])/1000,'b--')
...
plt.savefig("fig8_2.png",dpi=300)
plt.savefig("fig8_2.pdf",dpi=300)
plt.savefig("fig8_2.eps",dpi=300)
plt.close()
# plt.show()
