...
def s1c_exam_solving_pic(fit_path,save_fname, lat_deg=56.1501667,lon_deg=46.1050833,hei_m=183.):
    spath="./.temp/"
...
#     num_frames=1
    ...
    for i in range(num_frames):
        ...
        AA=SC.transform_to(AltAz(obstime=date_obs, location=s1c_site,temperature=20*u.deg_C,pressure=1013*u.hPa,
                                                   relative_humidity=0.5,obswl=630.0*u.nm))

        AZ=AA.az.rad
        ALT=AA.alt.rad

        filt_mask=np.where(ALT>80.0*np.pi/180)
        ALT=ALT[filt_mask]
        AZ=AZ[filt_mask]
...
movie_fname="s1c_140824_solve.mp4"
png_prefix, num_frames= s1c_exam_solving_pic(fit_path,save_fname)
...
movie_fname="s1c_140826_solve.mp4"
png_prefix, num_frames= s1c_exam_solving_pic(fit_path,save_fname)
make_movie_from_pngs(png_prefix, num_frames, movie_fname)
