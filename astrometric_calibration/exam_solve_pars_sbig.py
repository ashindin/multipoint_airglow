...
def sbig_exam_solving_pic(fit_path,save_fname, lat_deg=56.1501667,lon_deg=46.1050833,hei_m=183.):
    spath="./.temp/"
...
#     num_frames=1
    ...
    for i in range(num_frames):
        ...
        if i==0:
            png_prefix=spath+"frame_sbig_"+str(date_obs.year)[2::]+"{0:0>2}".format(date_obs.month)+"{0:0>2}".format(date_obs.day)+"_"
        ...
        filt_mask=np.where(ALT>85.0*np.pi/180)
        ALT=ALT[filt_mask]
        AZ=AZ[filt_mask]
...
movie_fname="sbig_160829_solve.mp4"
png_prefix, num_frames= sbig_exam_solving_pic(fit_path,save_fname)
